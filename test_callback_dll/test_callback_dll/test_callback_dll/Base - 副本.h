...
namespace Base
{
	...
	//extern int depth_H = 250;
	//extern int depth_W = 250;
	//extern float actual_distance = 520;//MM毫米 现在的这个距离这个就是差不多52公分长250个像素点 X横向上的距离 本身depth的那个就是MM的,这样差不多一个像素点就代表了2MM
	//extern float pixel_distance = actual_distance / depth_W;
	//extern float pixel_distance_square = pixel_distance * pixel_distance;
	//extern int conversion_rate = 1000 * 1000;////立方毫秒 --》 立方分米
	//extern int pox_dir_num = 0;
	...
	//extern std::vector<cv::Point2i> NeihborPos;////8邻域
	//extern std::map<int, std::vector<cv::Point2i>> MaskNum;////中心点的附近的几个像素邻域点
